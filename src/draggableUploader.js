...
class DraggableUploader extends React.Component{
	...
	render(){
		...
		const {loadedFiles, hover} = this.state;
		const rendererText = () => {
			if(!hover){
				return <div className = "helper-text">Drag and Drop Images Here</div>
			} else {
				return <div className = "helper-text">File Is Being Added</div>
			}
		}
		...
		return(
			<div className = "inner-container" style = {{display: "flex", flexDirection: "column"}}>
				...
				<AnchorButton text="Upload" intent={Intent.SUCCESS} onClick = {this.onUpload.bind(this)}/>
			</div>
		);
	}
}
